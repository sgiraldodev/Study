...
and one user who posted two tweets in 2022. The query groups the users by the number of tweets 
they posted and displays the number of users in each group.
...
WITH per_user AS (
  ...
  WHERE tweet_date >= '2022-01-01'
    AND tweet_date <  '2023-01-01'   -- rango cerrado-abierto para todo 2022
  GROUP BY user_id
)
SELECT
  tweet_bucket,          -- # de tweets por usuario
  COUNT(*)   AS users_num         -- # de usuarios con ese # de tweets
FROM per_user
